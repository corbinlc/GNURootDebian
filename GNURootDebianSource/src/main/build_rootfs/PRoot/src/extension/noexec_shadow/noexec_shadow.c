...
static int shadow_to_root(Tracee *tracee, char root_path[PATH_MAX])
{
	...
	status = my_readlink(tracee, shadow_link_path, shadow_path);
...
static int handle_rename_core(Tracee *tracee, char old_path[PATH_MAX], char new_path[PATH_MAX])
{
	...
	status = my_readlink(tracee, new_shadow_link_path, shadow_path);
	if (status == 0) {
	...
	status = my_readlink(tracee, old_shadow_link_path, shadow_path);
	if (status < 0)
		return 0;
